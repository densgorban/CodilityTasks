...
public class MergeWords {
    ...
    String mergeWords(String p, String q) {
        ...
        for (int i = 0; i < p.length(); i++) {
            ...
            Character qChar = q.charAt(i);
            if(charsStats.get(pChar).intValue() == charsStats.get(qChar).intValue())
                if (result.contains(qChar)) result.add(qChar);
                else result.add(pChar);
            else if(charsStats.get(pChar) > charsStats.get(qChar)) result.add(pChar);
            else
                result.add(qChar);
        }
    ...
    private Map<Character, Integer> calculateCharsNumber(String line, Map<Character, Integer> charsStats) {
        if (line.isEmpty()) return charsStats;
        else {
            char ch = line.charAt(0);
            String newLine = line.replaceAll(String.valueOf(ch), "");
            int numberOfChars = line.length() - newLine.length();
            charsStats.put(ch, numberOfChars);
