...
class Filters {
    ...
    public static List<Apple> filterByColor(List<Apple> apples) {
       return  apples.stream().filter(x -> x.color().equals("green")).collect(Collectors.toList());
    }
    ...
    public static Map<String, List<Apple>> groupByColor(List<Apple> apples) {
        ...
        for (Apple a: apples) {
            result.putIfAbsent(a.color(), new ArrayList<>()).add(a);
        }
        return result;
    }
    ...
    public static Optional<Apple> maxWeight(List<Apple> apples) {
        return apples.stream().max((o1, o2) -> (int) (o1.weight() - o2.weight()));
    }
